...
def show(request):
  if request.method == "POST":
       ...
       cartitem(customer=customer,orid=orid,image=image,name=name).save()
  id=request.session['id']
  cr = jew.objects.all()
  return render(request,'show.html',{'id':id,'cr':cr})
#coustomer reg
def usreg(request):
    if request.method == "POST":
        name = request.POST.get('name')
...
#cart
def cart(request,pk):
   cr = cartitem.objects.filter(customer=pk)
   id=request.session['id']

   if cr and id:
      return render(request,"cart.html",{'cr':cr,'id':id})
   else:
       return HttpResponse("Cart or customer not found.")
...
def button_view(request):
    if request.method == 'POST':
        ...
        if request.session.get('button_pressed'):
            # Button has already been pressed, set a flag
            request.session['button_already_pressed'] = True
        else:
            # Button is being pressed for the first time, set the flag and mark the button as pressed
            request.session['button_already_pressed'] = False
            request.session['button_pressed'] = True
